...
def get_applicable_coupons(restaurant_id):
    """
    Fetches all active coupons for a specific restaurant, including platform-wide ones.
    """
    try:
        # Find coupons that are active AND (are platform-wide OR belong to the specific restaurant)
        coupons = Coupon.query.filter(
            Coupon.is_active == True,
            or_(Coupon.restaurant_id == None, Coupon.restaurant_id == restaurant_id)
        ).all()
        
        coupons_data = [{'code': c.code} for c in coupons]
        return jsonify(coupons_data), 200
...
# ✅ START: NEW BULK UPLOAD ENDPOINT
...
def bulk_upload_menu():
    ...
    Expected Excel format: | Category | Name | Description | Price |
    """
    ...
    try:
        ...
        category_cache = {} # Cache to avoid repeated DB lookups for categories
        items_to_add = []
        items_added_count = 0

        # Skip header row (min_row=2)
        for row in sheet.iter_rows(min_row=2, values_only=True):
            category_name, item_name, description, price = row
            ...
            if category_name in category_cache:
                ...
                if category:
                    ...
                    db.session.add(new_category)
                    db.session.flush() # Flush to get the new ID without a full commit
        ...
        return jsonify({"message": "An error occurred while processing the file. Please check the format."}), 500
# ✅ END: NEW BULK UPLOAD ENDPOINT
